...
type TicketCategory = "Billing" | "Technical" | "Domain" | "Hosting" | "General";
      ...
      messages: [
        {
          ...
          author: "Jane Smith",
          authorType: "customer",
          message: "After the recent server migration, my email accounts aren't working. I can't send or receive emails through Outlook.",
      ...
      messages: [
        {
          ...
          author: "Bob Wilson",
          authorType: "customer",
          message: "I need help installing an SSL certificate on my website business.net",
        ...
        },
        {
          ...
          author: "Alex Rivera",
          authorType: "agent",
          message: "I've successfully installed your SSL certificate. Your website is now secure with HTTPS. Please allow up to 24 hours for full propagation.",
          ...
          <Card className="shadow-md">
            <CardContent className="pt-6">
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <Label className="body-sm font-semibold text-gray-700">Category</Label>
                  <p className="body-sm text-gray-900">{selectedTicket.category}</p>
                </div>
                <div>
                  <Label className="body-sm font-semibold text-gray-700">Status</Label>
                  <p className="body-sm text-gray-900">{selectedTicket.status}</p>
                </div>
                <div>
                  <Label className="body-sm font-semibold text-gray-700">Priority</Label>
                  <p className="body-sm text-gray-900">{selectedTicket.priority}</p>
                </div>
                <div>
                  <Label className="body-sm font-semibold text-gray-700">Assigned Agent</Label>
                  <p className="body-sm text-gray-900">{selectedTicket.agent || "Unassigned"}</p>
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="shadow-md">
            ...
            <CardContent className="space-y-4">
              {selectedTicket.messages.map((message) => (
                ...
                >
                  <div className={`max-w-2xl p-4 rounded-lg space-y-2 ${
                    ...
                    <div className="flex items-center space-x-2">
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center ${
                        message.authorType === "customer" ? "bg-white/20" : "bg-gray-300"
                      }`}>
                        <User className="h-3 w-3" />
                      </div>
                      <span className="body-sm font-semibold">{message.author}</span>
                      <span className={`body-sm ${
                        message.authorType === "customer" ? "text-white/80" : "text-gray-500"
                      }`}>
                        {message.timestamp}
                      </span>
                    </div>
    ...
    <AppShell>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button className="bg-primary hover:bg-primary/90" onClick={() => setActiveTab("create")}>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            ...
            <Card className="shadow-md">
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="category" className="body-sm font-semibold">Category</Label>
                    <Select>
                      <SelectTrigger>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="priority" className="body-sm font-semibold">Priority</Label>
                    <Select>
                      <SelectTrigger>
                ...
                <div className="space-y-2">
                  <Label htmlFor="subject" className="body-sm font-semibold">Subject</Label>
                  <Input
                    id="subject"
                ...
                <div className="space-y-2">
                  <Label htmlFor="description" className="body-sm font-semibold">Description</Label>
                  <Textarea
                    id="description"
                ...
                {/* File Upload Area */}
                <div className="space-y-2">
                  <Label className="body-sm font-semibold">Attachments (Optional)</Label>
                  <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-primary/50 transition-colors">
                    ...
                    <p className="text-xs text-gray-500 mt-2">
                      Maximum 5 files, 5MB each. Supported formats: JPG, PNG, PDF, DOC, TXT
                    </p>
                  </div>
                </div>
                ...
                {attachments.length > 0 && (
                  <div className="space-y-2">
                    <Label className="body-sm font-semibold">Attached Files</Label>
                    <div className="space-y-2">
                      {attachments.map((attachment) => (
                        <div key={attachment.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
                          <div className="flex items-center space-x-3">
                            <Paperclip className="h-4 w-4 text-gray-500" />
                            <div>
                              <p className="body-sm font-medium">{attachment.name}</p>
          ...
          <TabsContent value="tickets" className="space-y-6">
            ...
            <div className="space-y-4">
              {filteredTickets.map((ticket) => (
                <Card key={ticket.id} className="shadow-md hover:shadow-lg transition-shadow cursor-pointer">
                  <CardContent className="pt-6">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-3">
                        <div className="text-right">
                          ...
                          <Badge className={`${getPriorityColor(ticket.priority)} mt-2`}>
                            {ticket.priority}
                          </Badge>
                        </div>
                        <Button 
          ...
          <TabsContent value="knowledge" className="space-y-6">
              ...
              <CardContent>
                <div className="grid gap-4">
                  {knowledgeArticles.filter(article => article.featured).map((article) => (
                    <Card key={article.id} className="border border-gray-200 hover:shadow-md transition-shadow cursor-pointer">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {["Domains", "Hosting", "Email", "Security", "Billing", "Technical"].map((category) => (
                <Card key={category} className="shadow-md hover:shadow-lg transition-shadow cursor-pointer">
                  <CardContent className="pt-6 text-center">
                    ...
                    <p className="body-sm text-gray-600 mb-4">
                      {knowledgeArticles.filter(a => a.category === category).length} articles
                    </p>
