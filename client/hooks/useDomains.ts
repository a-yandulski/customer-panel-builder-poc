...
export function useDomains() {
  ...
  const fetchDomains = useCallback(
    async (newFilters?: Partial<DomainFilters>) => {
  ...
  const updateDomain = useCallback(
    async (domainId: string, updates: Partial<Domain>) => {
      try {
        const response = await api.patch<{ domain: Domain }>(
          `/domains/${domainId}`,
          updates,
        );
        ...
        setDomains((prev) =>
          prev.map((domain) =>
            domain.id === domainId ? { ...domain, ...response.domain } : domain,
          ),
        );
  ...
  const toggleAutoRenew = useCallback(
    async (domainId: string) => {
      ...
      setDomains((prev) =>
        prev.map((d) =>
          d.id === domainId ? { ...d, autoRenew: newAutoRenew } : d,
        ),
      );
      ...
      try {
        ...
        setDomains((prev) =>
          prev.map((d) =>
            d.id === domainId ? { ...d, autoRenew: domain.autoRenew } : d,
          ),
        );
      }
    },
  ...
  const updateNameservers = useCallback(
    async (domainId: string, nameservers: string[]) => {
      try {
...
export function useDNSRecords(domainId: string | null) {
  ...
  const fetchDNSRecords = useCallback(
    async (recordType?: string, sortBy?: string) => {
      if (!domainId) return;
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Failed to fetch DNS records";
        setError(errorMessage);
        toast({
          title: "Error",
          description: errorMessage,
